...
ELSE IF @pChar = 'P'      --모든 아이들의 화폐 소지 개수 다음과 같이 늘려주기
	BEGIN  
		SELECT @pMinute = CONVERT(INT, SUBSTRING(CONVERT(char(8), GETDATE(), 112),7,2))   -- 날짜의 '분'을 int형으로 변환하여 선언
...
ELSE
	BEGIN		
		...
		SELECT '성공!' AS [결과], @@Trancount AS [트랜젝션 수], @@ERROR AS [ERROR]	
			...
			ELSE   --'D'를 제외한 
				BEGIN
					SELECT ChildNm ,CNT100 AS [백원], CNT500 AS [오백원], CNT1000 AS [천원], CNT5000 AS [오천원], CNT10000 AS [만 원]
				    FROM dbo.Test3_Child_이완상
				END
	END